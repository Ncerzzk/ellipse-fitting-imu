...
for i=1:1:size(x,1)
    tempPn={x(i)^2,y(i)^2,z(i)^2,x(i),y(i),z(i),1};
    tempPn=cell2mat(tempPn);
    tempPn2=tempPn'*tempPn;
    A=A+tempPn2;
end
B=gaussLinearEquation(A,zeros(7,1))
B(7,:)=zeros(1,7);
B
s=null(B)
...
a=s(1);b=s(2);c=s(3);d=s(4);e=s(5);f=s(6);g=s(7);
...
sum=0;
for i=1:1:size(x,1)
    sum=(((x(i)-xo)*kx)^2+((y(i)-yo)*ky)^2+((z(i)-zo)*kz)^2-1)^2+sum;
